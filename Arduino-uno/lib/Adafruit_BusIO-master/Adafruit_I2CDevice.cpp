...
bool Adafruit_I2CDevice::begin(bool addr_detect) {
  _wire->begin();
  Serial.println("CONECTO EL WIRE");
  _begun = true;
...
bool Adafruit_I2CDevice::detected(void) {
  ...
  if (!_begun && !begin()) {
    Serial.println("El I2C parece no estar incializado");
    return false;
  }

  // A basic scanner, see if it ACK's
  _wire->beginTransmission(_addr);
  Serial.println("Inicio transmiision de wire");
  ...
  if (_wire->endTransmission() == 0) {
    Serial.println("La transmision se cerro correctamente");
#ifdef DEBUG_SERIAL
