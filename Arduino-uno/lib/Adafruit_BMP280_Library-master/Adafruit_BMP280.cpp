...
bool Adafruit_BMP280::begin(uint8_t addr, uint8_t chipid) {
  if (spi_dev == NULL) {
    ...
    Serial.println("Prueba I2C");
    if (!i2c_dev->begin())
      Serial.println("Exploto por el I2C");
      return false;
  } else {
    // SPI mode
    if (!spi_dev->begin())
      Serial.println("ES POR SPI");
      return false;
  }
  ...
  if (_sensorID != chipid)
    Serial.println("ES POR EL ID");
    return false;

  Serial.println("Funciona I2C");
  readCoefficients();
  Serial.println("Funciona COF");
  // write8(BMP280_REGISTER_CONTROL, 0x3F); /* needed? */
  setSampling();
  Serial.println("Funciona sampling");
  delay(100);
  Serial.println("ESTA Saliendo de BEGIN");
  return true;
}
